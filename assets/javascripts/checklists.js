...
document.addEventListener('DOMContentLoaded', function() {
  var queryForm = document.querySelector('form#query_form');
  var submitLink = document.querySelector('a.icon.icon-checked');

  if (queryForm && submitLink) {
    ...
    newCheckbox.name = 'c[]';
    newCheckbox.value = 'checklists';
    newCheckbox.id = 'show_checklists';

    var labelText = document.createTextNode(' Checklist');
    newLabel.appendChild(newCheckbox);
    newLabel.appendChild(labelText);

    var lastNotesTr = queryForm.querySelector('tr td label.inline input[value="last_notes"]');

    if (lastNotesTr) {
      lastNotesTr.closest('td').appendChild(newLabel);
    }

    var isChecked = localStorage.getItem('checklistsVisible') === 'true';
    newCheckbox.checked = isChecked;

    if (isChecked) {
      toggleChecklists(true); 
    }

    newCheckbox.addEventListener('change', function () {
      localStorage.setItem('checklistsVisible', newCheckbox.checked ? 'true' : 'false');
    });
    ...
    submitLink.addEventListener('click', function (e) {
      ...
      if (newCheckbox.checked) {
        toggleChecklists(true);  
        localStorage.setItem('checklistsVisible', 'true');
      } else {
        toggleChecklists(false);  
        localStorage.setItem('checklistsVisible', 'false');
      }
    });
  }
});

function toggleChecklists(visible) {
  jQuery('tr.issue').each(function() {
    var issueRow = jQuery(this);
    var issueId = issueRow.attr('id').split('-')[1];

    var checklistForIssue = jQuery('#checklists_container').find('.checklist-container[data-issue-id="' + issueId + '"]');

    if (checklistForIssue.length) {
      var checklistContainerClone = checklistForIssue.clone();
      checklistContainerClone.attr('id', 'checklist_for_issue_' + issueId);

      var descriptionRow = issueRow.next('tr.description');

      if (descriptionRow.length) {
        descriptionRow.append(checklistContainerClone);
      } else {
        var newDescriptionRow = jQuery('<tr class="description"><td colspan="9"></td></tr>');
        newDescriptionRow.find('td').append(checklistContainerClone);
        issueRow.after(newDescriptionRow);
      }

      checklistContainerClone.toggle(visible);

      checklistContainerClone.find('.checklist-items').each(function() {
        jQuery(this).checklist();
      });
    }
  });

  jQuery('#checklists_container').remove();
}
